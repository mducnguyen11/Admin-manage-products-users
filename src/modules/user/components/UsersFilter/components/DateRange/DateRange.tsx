...
import { formateDateToTimeStamp, formatTimeStampToISOSString } from 'utils/formatTime';
...
const FilterDateRange = (props: Props) => {
  ...
  const handleDateSelect = (
    ...
    if (a) {
      if (a.from) {
        console.log('from :', formatTimeStampToISOSString(formateDateToTimeStamp(a.from)));
        tt.push(formatTimeStampToISOSString(formateDateToTimeStamp(a.from)));
      }
      if (a.to) {
        tt.push(formatTimeStampToISOSString(formateDateToTimeStamp(a.to)));
      }
    }
    return tt;
  };
  useEffect(() => {
    ...
    xx.forEach((l, i) => {
      ...
      if (t.toDateString() !== 'Invalid Date') {
        b.push(formatTimeStampToISOSString(formateDateToTimeStamp(t)));
      }
    });
    props.onChange({
      date_range: b,
